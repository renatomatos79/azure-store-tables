...
const TABLE_TO_BE_DELETED = "DeleteTable";
const MAX_RETRIES = 3;
...
console.log("Settings: ", { accountName, accountKey, TABLE_NAME });
...
async function main() {
  ...
  Promise.all(
    USERS.map(async (user) => {
      console.log("Checking entity: ", user);
      const row = await getEntity(
        tableClient,
        user,
        MAX_RETRIES,
  ...
  Promise.all(toBeDeleted.map(user => deleteEntity(tableClient, user, MAX_RETRIES, RETRY_DELAY_IN_MS)));
